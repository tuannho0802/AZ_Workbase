 ...
 * (Phase 4 - phụ trách phụ, xem `periodic-task-secondary-assignee.entity.ts`)
 * đều đã có, nhưng CỐ TÌNH không khai `@OneToMany` ngược lại ở đây - quan hệ
 * được truy vấn qua `PeriodicTaskLinksService`/`PeriodicTaskCustomersService`/
 * `PeriodicTaskSecondaryAssigneesService` (join tường minh qua query
 * builder), không cần eager/lazy relation 2 chiều làm phức tạp thêm entity
 * chính vốn đã được nhiều module khác tham chiếu.
 */
