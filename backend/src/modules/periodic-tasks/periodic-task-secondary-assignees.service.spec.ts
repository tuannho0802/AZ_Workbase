...
describe('PeriodicTaskSecondaryAssigneesService', () => {
  ...
  describe('addSecondaryAssignee', () => {
    it('ném BadRequestException nếu userId trùng primaryAssigneeId (spec bắt buộc PLAN mục 6: "chính khác phụ")', async () => {
      await expect(
        service.addSecondaryAssignee(taskId, { userId: primaryAssigneeId }, employeeUser.id, employeeUser.role, 'own'),
    ...
    it('ném BadRequestException nếu user không tồn tại hoặc đã bị khoá', async () => {
      ...
      await expect(
        service.addSecondaryAssignee(taskId, { userId: 99 }, employeeUser.id, employeeUser.role, 'own'),
      ).rejects.toThrow(BadRequestException);
    ...
    it('ném ConflictException nếu đã là Phụ trách phụ rồi (spec bắt buộc: "unique constraint - không thêm trùng")', async () => {
      ...
      await expect(
        service.addSecondaryAssignee(taskId, { userId: 5 }, employeeUser.id, employeeUser.role, 'own'),
    ...
    it('thêm thành công + trả lại danh sách User đang là phụ trách phụ', async () => {
      ...
      const result = await service.addSecondaryAssignee(taskId, { userId: 5 }, employeeUser.id, employeeUser.role, 'own');
  ...
  describe('removeSecondaryAssignee', () => {
    it('ném NotFoundException nếu người này không phải Phụ trách phụ của Task', async () => {
      ...
      await expect(
        service.removeSecondaryAssignee(taskId, 5, employeeUser.id, employeeUser.role, 'own'),
    ...
    it('gỡ thành công khi tồn tại', async () => {
      ...
      const result = await service.removeSecondaryAssignee(taskId, 5, employeeUser.id, employeeUser.role, 'own');
