...
describe('PeriodicTaskAccessHelper', () => {
  describe('applyViewFilter', () => {
    it('ADMIN: không áp thêm bất kỳ điều kiện lọc nào (xem tất cả)', () => {
      ...
      const result = PeriodicTaskAccessHelper.applyViewFilter(qb, 1, Role.ADMIN, 'all');
    ...
    it('scope=all (kể cả role tuỳ chỉnh): không áp thêm điều kiện lọc nào', () => {
      ...
      PeriodicTaskAccessHelper.applyViewFilter(qb, 1, 'custom_manager', PermissionScope.ALL);
    ...
    it('scope=department: lọc theo phòng ban có dòng department_managers cho chính mình', () => {
      ...
      PeriodicTaskAccessHelper.applyViewFilter(qb, 42, Role.MANAGER, PermissionScope.DEPARTMENT);
    ...
    it('scope=own (EMPLOYEE): lọc theo (createdById = mình) OR (primaryAssigneeId = mình)', () => {
      const { qb, calls } = makeFakeQueryBuilder();

      PeriodicTaskAccessHelper.applyViewFilter(qb, 7, Role.EMPLOYEE, PermissionScope.OWN);
  ...
  describe('canDelete', () => {
    it('ADMIN -> true', () => {
      expect(PeriodicTaskAccessHelper.canDelete({} as any, 1, Role.ADMIN)).toBe(true);
    });

    it('ASSISTANT -> false (chỉ Admin được xoá)', () => {
      expect(PeriodicTaskAccessHelper.canDelete({} as any, 1, Role.ASSISTANT)).toBe(false);
    });
  ...
  describe('canManageTask', () => {
    it('ADMIN -> luôn true bất kể dữ liệu task', () => {
      expect(PeriodicTaskAccessHelper.canManageTask({} as any, 1, Role.ADMIN)).toBe(true);
    });

    it('scope=all -> luôn true bất kể dữ liệu task', () => {
      expect(PeriodicTaskAccessHelper.canManageTask({} as any, 1, 'custom_manager', [], 'all')).toBe(true);
    });
    ...
    it('scope=department -> true nếu departmentId của task nằm trong managerDepartmentIds truyền vào', () => {
      ...
      expect(PeriodicTaskAccessHelper.canManageTask(task, 1, Role.MANAGER, [3, 5, 8], 'department')).toBe(true);
    });

    it('scope=department -> false nếu departmentId KHÔNG nằm trong managerDepartmentIds', () => {
      const task: any = { departmentId: 99 };
      expect(PeriodicTaskAccessHelper.canManageTask(task, 1, Role.MANAGER, [3, 5, 8], 'department')).toBe(false);
    });
    ...
    it('scope=department -> false nếu không truyền managerDepartmentIds (mặc định rỗng - phòng thủ)', () => {
      const task: any = { departmentId: 5 };
      expect(PeriodicTaskAccessHelper.canManageTask(task, 1, Role.MANAGER, undefined as any, 'department')).toBe(false);
    });

    it('own -> true nếu là người tạo (createdById)', () => {
      const task: any = { createdById: 7, primaryAssigneeId: 99 };
      expect(PeriodicTaskAccessHelper.canManageTask(task, 7, Role.EMPLOYEE)).toBe(true);
    });
    ...
    it('own -> false nếu không phải người tạo, không phải phụ trách chính', () => {
      const task: any = { createdById: 99, primaryAssigneeId: 98 };
      expect(PeriodicTaskAccessHelper.canManageTask(task, 7, Role.EMPLOYEE)).toBe(false);
    });

    it('MANAGER + không có scope -> KHÔNG fallback cứng theo Role, kiểm tra theo nhánh own giống mọi role khác', () => {
      const task: any = { departmentId: 5, createdById: 99, primaryAssigneeId: 98 };
      expect(PeriodicTaskAccessHelper.canManageTask(task, 1, Role.MANAGER, [5], null)).toBe(false);
      const ownedTask: any = { departmentId: 5, createdById: 1 };
      expect(PeriodicTaskAccessHelper.canManageTask(ownedTask, 1, Role.MANAGER, [5], null)).toBe(true);
    });
  });
});
