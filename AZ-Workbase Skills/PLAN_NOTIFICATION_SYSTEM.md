...
> **Trạng thái:** DỰ THẢO v2. **Phase 0 + 1 + 3 (BE) + 4 (FE, điều hướng cơ bản) đã xong (2026-09-21)** — xem `WORKFLOW_LOG.md`. **Phase M1 (BE Thông báo thủ công) đã xong (2026-09-22)** — CRUD đầy đủ theo yêu cầu chủ dự án (mở rộng hơn dự kiến gốc chỉ 2 permission `send/view`, xem mục M1 bên dưới và `PERMISSIONS.md` §2.12). **Phase 2 (móc Task) đã CODE xong (2026-09-22)** — `emit()` gắn đủ ở `create/update/lock/unlock/remove` (`periodic-tasks.service.ts`) + 3 service phụ (secondary-assignees/checklist-items/customers). **Phase M2 (FE Thông báo thủ công) đã CODE xong (2026-09-22), verify sạch (2026-09-22)** — 2 trang `/thong-bao/gui` + `/thong-bao/da-gui`, `SendBroadcastModal`, nav gating. ✅ **Phase 2 + M2 đã chạy lại đầy đủ và verify sạch (2026-09-22):** BE `tsc --noEmit` sạch, `nest build` sạch, **46/46 suite / 862/862 test pass** (không regression). FE `next build` sạch (Turbopack, cả 2 route `/thong-bao/gui` + `/thong-bao/da-gui` lên đúng), `npx vitest run` **10/10 file / 78/78 test pass**. ⚠️ **Đã fix 2 bug phát hiện khi verify lại:** (1) nội dung trang "đã gửi" từng bị ghi nhầm vào file `thong-bao/gui/page.tsx` khiến `/thong-bao/da-gui` 404 — xem `PERMISSIONS.md` §2.12; (2) endpoint gửi phụ thuộc `NOTIFICATIONS_ENABLED=true` nhưng biến này chưa có trong `.env.development.example`, gây 400 "Tính năng thông báo đang tắt" nếu máy dev thiếu — đã bổ sung vào file ví dụ, chủ dự án cần tự thêm vào `.env.development` thật. Mục 11 vẫn chờ chủ dự án chốt (Phase 1 dùng đề xuất mặc định). **Đã lệch plan:** 5.1/11.13 `is_read` là cột ghi được (không phải cột sinh) — lý do trong WORKFLOW_LOG.
