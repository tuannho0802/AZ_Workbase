...
| `assignment_groups.manage` | assignment_groups | **MỚI (2026-09-10, Phase 4 PLAN Assignment Group).** CRUD "Quản lý phụ trách" (`/assignment-groups`) — bảng cấu hình toàn cục "nhóm phụ trách X gồm N Phòng ban (BẮT BUỘC ≥1) + N Vị trí (TUỲ CHỌN)", thay thế hardcode FE dò tên phòng ban. `supportsScope=false`. Seed mặc định: Admin/Assistant = `all`. Riêng `GET /assignment-groups/:key/users` KHÔNG cần permission này — chỉ cần đăng nhập (data hỗ trợ dropdown, giống triết lý `my-permissions`). Seed sẵn 2 config hệ thống `sales`/`marketing` (migration `1781100000000-CreateAssignmentGroupConfigs.ts`, auto map phòng ban theo tên chứa "kinh doanh"/"marketing", không xoá được). ⚠️ **CHƯA có nơi nào ở FE gọi `resolveUsers()` này** — `customers/page.tsx` vẫn đang dùng hardcode cũ, API mới chỉ tồn tại song song, chờ quyết định chuyển đổi riêng |
...
- **`link-group-managers.controller.ts` + `link-group-managers.service.ts` (MỚI — "Quản lý chính/phụ"
  ...
  - **FE — trang `/nhom-toi-quan-ly` (MỚI, fix 2026-09-11):** trước đây trang này KHÔNG có route guard
    nào (hiện với MỌI role đã đăng nhập bất kể Admin tắt quyền gì qua `/phan-quyen`) — khác biệt so với
    MỌI trang khác trong app. Đã thêm gate `permission: 'link_groups.my_managed'` ở `nav-config.tsx` +
    `useEffect` redirect ở chính `page.tsx` (cùng pattern `attendance-device/page.tsx`). Đây CHỈ là
    permission "được thấy trang", KHÔNG thay thế mô hình quyền tài nguyên cụ thể ở trên (dữ liệu bên
    trong trang vẫn luôn lọc đúng theo `managed-by-me`, nút "Quản lý" thêm/xoá quản lý phụ vẫn luôn gate
    bằng `canManage`/`canEditSecondaryManagers` ở BE, không đổi).
...
| `periodic_tasks.trash_manage` | false (nhị phân) | bật/tắt/tắt/tắt | Thùng rác Task: xem Task đã xoá mềm + KHÔI PHỤC + xoá VĨNH VIỄN + dọn sạch (mặc định chỉ Admin, migration `1784300000000`) |
