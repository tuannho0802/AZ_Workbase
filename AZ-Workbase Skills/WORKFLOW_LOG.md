...
## [2026-09-25 09:08 UTC] | Fix `Cannot find name 'WeekGroupedRequests'` + hoàn tất render `duyet-phep/page.tsx` (3 tab lazy-per-week) | Status: Success

**Actor:** Agent

**Bối cảnh:** Tiếp nối phiên trước (đã đổi `nghi-phep/page.tsx` sang `WeeklyLazySection` xong, và đã viết
xong toàn bộ state/fetch layer 3-tab của `duyet-phep/page.tsx` — `fetchTab`/`fetchPending`/`fetchHistory`/
`fetchTrash`/`fetchWeekPending`/`fetchWeekHistory`/`fetchWeekTrash`/`handleSoftDelete`/`handleRestore`/
`handleHardDelete`/`trashColumns`/`TrashMobileCard`) nhưng dừng giữa chừng ở phần render (`tabItems`) —
build báo lỗi `Cannot find name 'WeekGroupedRequests'` vì component cũ đã bị xoá khỏi import nhưng JSX vẫn
gọi tới nó, cùng 2 biến `filteredPending`/`filteredHistory` cũng đã bị xoá theo.

**Root cause (verify bằng code thật, không suy diễn):**
- `git clone` lại `main` ở commit `0c02bac` (đúng bản người dùng vừa push) — xác nhận `WeeklyLazySection`
  đã được import sẵn ở đầu file, và toàn bộ state/fetch layer đã đúng như báo cáo phiên trước.
- Lỗi TS2304 (`WeekGroupedRequests` không tồn tại) chỉ nằm ở khối `tabItems` (JSX) — phần DUY NHẤT chưa
  được cập nhật để khớp layer state/fetch mới.
- Kèm 3 biến "mồ côi" khác trong cùng khối JSX: `pendingRequests.length` (badge đếm), `pendingDeptOptions`/
  `historyDeptOptions` (dropdown Phòng ban) — không còn được khai báo ở đâu trong file (đã bị xoá khi dọn
  layer state cũ ở phiên trước nhưng chưa cập nhật nơi dùng).

**Solution:**
- Viết lại toàn bộ khối `tabItems` trong `frontend/src/app/(dashboard)/duyet-phep/page.tsx`, mirror ĐÚNG
  pattern `nghi-phep/page.tsx` (đã verify tuần trước): thay `WeekGroupedRequests` bằng `WeeklyLazySection`
  cho cả 2 tab "Chờ phê duyệt" và "Lịch sử phê duyệt", nối đúng `weeks`/`fetchWeek`/`resetKey`/`pagination`
  vào state tương ứng (`pendingState`/`historyState`) và các hàm `fetchWeekPending`/`fetchWeekHistory`/
  `fetchPending`/`fetchHistory` đã có sẵn từ phiên trước.
- Thêm MỚI tab thứ 3 "Thùng rác" (`key: 'trash'`, gate quyền `canDelete`) dùng `WeeklyLazySection` +
  `trashColumns` + `TrashMobileCard` + `fetchWeekTrash`/`fetchTrash` (state/handler đã có sẵn, chỉ thiếu
  JSX gọi tới) — đúng phần "chưa làm" mà phiên trước đã báo còn thiếu.
- `Tabs` gắn thêm `onChange` để set `trashTabLoaded=true` lần đầu mở tab "Thùng rác" (lazy-load, khớp
  `useEffect` đã viết sẵn từ phiên trước chỉ chạy `fetchTrash` khi `trashTabLoaded` true).
- Badge đếm "Chờ phê duyệt" đổi `pendingRequests.length` (mảng client không còn tồn tại) sang
  `pendingState.total` (tổng số PHA 1 trả về từ BE).
- Dropdown "Phòng ban" ở cả 3 tab đổi `pendingDeptOptions`/`historyDeptOptions` (không tồn tại) sang dùng
  chung `departmentOptions` (đã có sẵn từ `useDepartments()`, khai báo đầu file).
- Dọn 2 biến `pendingTableWidth`/`historyTableWidth` (chỉ dùng cho `WeekGroupedRequests` cũ, không còn cần
  vì `WeeklyLazySection` tự set `scroll`) — thay bằng comment giải thích, không xoá logic gì khác.

**Verify thật đã chạy (sandbox, clone mới từ `main`, commit `0c02bac`, sau khi sửa):**
- `npx tsc --noEmit` — đúng 5 lỗi baseline PRE-EXISTING (`logo.png` x4, `CountBadge.tsx` styled-jsx),
  KHÔNG còn lỗi `WeekGroupedRequests`/`filteredPending`/`filteredHistory`/`pendingRequests`/
  `pendingDeptOptions`/`historyDeptOptions` nào. Đã `grep` xác nhận không còn tham chiếu nào tới các tên cũ
  trong file.
- `npx eslint "src/app/(dashboard)/duyet-phep/page.tsx"` — 31 lỗi `no-explicit-any` (so với 45 lỗi TRƯỚC
  khi sửa, đo bằng `git stash`/`git stash pop` trên cùng máy) — toàn bộ là `any` PRE-EXISTING ở các cột
  bảng (`render: (_: any, record...)`), cùng class với style sẵn có toàn file, không phải lỗi mới.
- `npx vitest run` — 19/19 file, 132/132 test pass (không đổi so với phiên trước).
- `git diff --stat` — chỉ 1 file đổi: `frontend/src/app/(dashboard)/duyet-phep/page.tsx` (+113/-18 dòng,
  đúng phạm vi khối `tabItems` + 2 dòng comment dọn biến thừa). `package-lock.json` bị `npm install` đổi
  ngoài ý muốn đã `git checkout` phục hồi lại trước khi báo cáo.

**Notes:**
> Chưa commit/push lên `main` — đính kèm diff trong chat, chủ dự án tự review + áp + deploy (đúng rule
> "không tự chạy migration/deploy" + để chủ dự án kiểm soát lịch sử commit).
> Việc còn lại (nếu có): UI 3 tab giờ đã khớp đầy đủ layer state/fetch đã viết ở phiên trước — không còn
> phần "dở dang" nào được biết tới ở `duyet-phep/page.tsx`/`nghi-phep/page.tsx` liên quan `WeeklyLazySection`.
